        ...
        <nav className="flex fixed w-full z-40 bg-white top-0 justify-center items-center px-10 lg:px-4 md:py-4">
            ...
            <div className="max-w-8xl flex justify-between flex-1 items-center md:hidden">
                ...
                <div className="flex font-semibold items-center">
                    <a className="flex items-center hover:text-primary-dark py-10 relative" href="#">
                        <svg viewBox="0 0 24 24" className="pr2 w-8 h-8 fill-current overflow-visible" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false" ><path d="M12,11.475 C10.5214286,11.475 9.32142857,10.299 9.32142857,8.85 C9.32142857,7.401 10.5214286,6.225 12,6.225 C13.4785714,6.225 14.6785714,7.401 14.6785714,8.85 C14.6785714,10.299 13.4785714,11.475 12,11.475 M12,1.5 C7.85357143,1.5 4.5,4.7865 4.5,8.85 C4.5,14.3625 12,22.5 12,22.5 C12,22.5 19.5,14.3625 19.5,8.85 C19.5,4.7865 16.1464286,1.5 12,1.5"></path></svg>  
                        Find a store
                        <div className="absolute opacity-0 w-full h-1 bg-primary-dark bottom-0"></div>
                    </a>
                    <Button className="ml-10" theme="white">Sign in</Button>
                    <Button className="ml-4" theme="black" href="">Join now</Button>
                </div>
            </div>

            <button onClick={menuButtonIsClicked} className={`hidden md:block ml-auto w-6 h-6 ${(menuIsOpen ? "pointer-events-none":"")}`}>
                <svg viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false"><path className="sb-hamburgerButton-middleLine-CLOSED sb-hamburgerButton-animation" d="M21,12.9H3c-0.5,0-0.9-0.4-0.9-0.9s0.4-0.9,0.9-0.9h18c0.5,0,0.9,0.4,0.9,0.9S21.5,12.9,21,12.9z"></path><path className="sb-hamburgerButton-topLine-CLOSED sb-hamburgerButton-animation sb-hamburgerButton-animation-outer-lines" d="M21,6.9H3C2.5,6.9,2.1,6.5,2.1,6S2.5,5.1,3,5.1h18c0.5,0,0.9,0.4,0.9,0.9S21.5,6.9,21,6.9z"></path><path className="sb-hamburgerButton-bottomLine-CLOSED sb-hamburgerButton-animation sb-hamburgerButton-animation-outer-lines" d="M21,18.9H3c-0.5,0-0.9-0.4-0.9-0.9s0.4-0.9,0.9-0.9h18c0.5,0,0.9,0.4,0.9,0.9S21.5,18.9,21,18.9z"></path><circle className="sb-icon-hover" fill="transparent" cx="50%" cy="50%" r="75%"></circle></svg>
            </button>

            <Transition.Root show={menuIsOpen}>
                <Dialog open={menuIsOpen} onClose={setMenuIsOpen}>
                    <Transition.Child
                        enter="transition-transform duration-300"
                        enterFrom="translate-x-0"
                        enterTo="-translate-x-full"
                        entered="-translate-x-full"
                        leave="transition-transform duration-300"
                        leaveFrom="-translate-x-full"
                        leaveTo="translate-x-0"
                        className="fixed top-0 bottom-0 -right-3/4 bg-gray-600 w-9/12 z-20 transform"
                    >
                        <div className="mt-28 flex flex-col">
                            <button onClick={subMenuButtonIsClicked}>Menu</button>
                            <Transition.Root show={subMenuIsOpen}>
                                <Dialog onClose={setMenuButton_SubMenuButton}>
                                    <Transition.Child
                                        enter="transition-transform duration-300"
                                        enterFrom="translate-x-0"
                                        enterTo="-translate-x-full"
                                        ...
                                        leaveFrom="-translate-x-full"
                                        leaveTo="translate-x-0"
                                        className="fixed top-0 bottom-0 -right-3/4 bg-black w-9/12 z-20 transform"
                                        >
                    ...
                    <Transition.Child
                        ...
                        leaveTo="opacity-0"
                    >
                        <Dialog.Overlay className="absolute top-0 bottom-0 right-0 left-0 bg-black w-full z-10"></Dialog.Overlay>
                    </Transition.Child>
                </Dialog>
            </Transition.Root>
